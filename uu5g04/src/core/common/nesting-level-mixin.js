...
export const NestingLevelMixin = {
  ...
  statics: {
    "UU5.Common.NestingLevelMixin": {
      ...
      errors: {
        incorrectRequestedNestingLevel: 'Component %s was used with property nestingLevel="%s", but it supports only one of: %s.',
        nestingLevelMismatch: 'Component %s with supported nesting levels %s cannot be nested into parent component %s which uses nesting level "%s". Component would have to support one of: %s.',
        nestingLevelMismatchExplicitProp: 'Component %s has prop nestingLevel="%s" and as such it cannot be nested into parent component %s which uses nesting level "%s". Component needs to use one of: %s.',
  ...
  getParentNestingLevel: function (parentNestingLevelComponent) {
    ...
    return component
      ? component.getNestingLevel()
      : Environment.nestingLevelList[0];
  },
  ...
  checkNestingLevel: function () {
    ...
    if (nestingLevel) {
      ...
      nestingLevelList = nestingLevelLocIndex > -1
        ? nestingLevelList.slice(nestingLevelLocIndex)
        : nestingLevelList
    }
    ...
    if (Environment.nestingLevelList.indexOf(nestingLevel) > Environment.nestingLevelList.indexOf(nestingLevelList[nestingLevelList.length - 1])) {
      this.showError(
        'incorrectRequestedNestingLevel', [this.getTagName(), nestingLevel, JSON.stringify(nestingLevelList)], {
          mixinName: "UU5.Common.NestingLevelMixin",
          context: {
    ...
    if (!this.getOpt('nestingLevelRoot')) {
      ...
      if (
        parentNestingLevelComponent && (!parentNestingLevelComponent.getOpt('nestingLevelWrapper')) &&
    ...
    if (nestingLevelEnvIndex < calculatedNestingLevelIndex) {
      if (this.props.nestingLevel) {
        ...
        this.showError(
          ...
          [this.getTagName(), JSON.stringify(nestingLevelList), parentNestingLevelComponent.getTagName(), parentNestingLevel, JSON.stringify(calculatedNestingLevelList)], {
            mixinName: "UU5.Common.NestingLevelMixin",
            context: {
              parent: {
                tagName: parentNestingLevelComponent.getTagName(),
