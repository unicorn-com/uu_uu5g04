...
export const EditableMixin = {
  ...
  startEditation(endEditationCallback, saveEditationCallback) {
    ...
    if (this.constructor.editableComponent){
      ...
      result = {
        content: this._createContentFromUU5StringTemplate(`<uu5string /><${this.constructor.editableComponent} id="${this.getId()}_editable-brick" component="\${this}" />`, {this: this}),
        header: <UU5.Bricks.Div>
          {this.getLsiComponent("header", "UU5.Common.EditableMixin", this.getTagName())}
          {docLink ? <UU5.Bricks.Link href={docLink} target={"_blank"} className={this.getClassName("docLink", "UU5.Common.EditableMixin")} key="docLink">
