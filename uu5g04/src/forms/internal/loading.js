  ...
  return (
    <span className={props.className}>
      <svg width="24" height="24" viewBox="0 0 100 100" version="1.1" xmlns="http://www.w3.org/2000/svg">
        ...
        <path key="path1" d="M50 10 a40 40 0 0 1 40 40" fill="none" stroke={(navigator.userAgent.match("Safari") && !navigator.userAgent.match("Chrome/") ? `url(${location.href.replace(/#.*/, "")}` :`url(` )+ `${"#gradient1_" + props.id})`} strokeWidth="20"/>
        <path key="path2" d="M90 50 a40 40 0 0 1 -40 40" fill="none" stroke={(navigator.userAgent.match("Safari") && !navigator.userAgent.match("Chrome/") ? `url(${location.href.replace(/#.*/, "")}` :`url(` )+ `${"#gradient2_" + props.id})`} strokeWidth="20"/>
        <path key="path3" d="M50 90 a40 40 0 0 1 -40 -40" fill="none" stroke={(navigator.userAgent.match("Safari") && !navigator.userAgent.match("Chrome/") ? `url(${location.href.replace(/#.*/, "")}` :`url(` )+ `${"#gradient3_" + props.id})`} strokeWidth="20"/> 
      </svg>
    </span>
  );
};
